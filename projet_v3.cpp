 ...
 *  INF-4101C
 *---------------------------------------------------------------------------------------------------
 * Pour compiler : g++ `pkg-config --cflags opencv` projet_v3.cpp `pkg-config --libs opencv` -o projet_v3
 *---------------------------------------------------------------------------------------------------
...
void median(Mat frame_gray, Mat frame1, int k){
  ...
  for (int lign = minval ; lign < maxlig ; lign++){
    for (int col = minval ; col < maxcol ; col++){
      ...
      int voisinage[256] = {0}; 
      //parcourir le voisinage et rescencer les occurences dans l'histo
      for (int x = lign-k/2 ; x < lign+k/2 ; x++){
        for(int y = col-k/2 ; y < col+k/2 ; y++){
